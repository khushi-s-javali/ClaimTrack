...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST["confirm_withdrawal"]) && $_POST["confirm_withdrawal"] == "true") {

        
        $sql = "UPDATE policy SET policy_status = ? WHERE policy_id = ?";
        ...
        $status = "Request For WithDraw";
        $policyid = $_POST['policy_id'];
         echo "Policy ID: " . $policyid; 
        
        $stmt->bind_param("ss", $status, $policyid);
        ...
        if ($stmt->errno) {
            ...
            header("Location: withdraw_policy.php");
            exit;
        }
...
<p>If you wish to withdraw this policy, please click the button below:</p>
...
<script>
    function requestWithdrawal() {
        ...
        var confirmation = confirm("Are you sure you want to withdraw this policy?");
        if (confirmation) {
